# TwinOak filter reviews

Important: Always check the sweep interval when comparing graphs! Some filters has a lot narrower passband, so the sweep might have been made smaller to increase the resolution!

## Sysmocom cf866.5-kt30
Source: [https://shop.sysmocom.de/868-863..870-MHz-cavity-filter-ISM-LoRa-SigFox-Helium/cf866.5-kt30](https://shop.sysmocom.de/868-863..870-MHz-cavity-filter-ISM-LoRa-SigFox-Helium/cf866.5-kt30)

Price: 53.54 EUR + shipping

![Sysmocom cf866.5-kt30 cavity filter spectrum analyzer test](sysmocom-cf866.5-kt30/sysmocom-cf866.5-kt30-tinysa.png)

## Elektronik Reich 869MHz SAW 28dBm u.FL
Source: [https://elektronikreich.de/product.html?id=2](https://elektronikreich.de/product.html?id=2)

Price 24.90 EUR + shipping (which is quite expensive)

Note: Test was done with poor cabling due to the u.fl connectors!

![alt text](elektronikreich-saw-869-28dbm-ufl/saw-869-28dbm-ufl-testsetup.png)

![Elektronik Reich 869MHz SAW 28dBm u.FL](elektronikreich-saw-869-28dbm-ufl/saw-869-28dbm-ufl-tinysa.png)

# Callboost 868MHz 4M
Source: [https://www.callboost.cn/product-callboost-cavity-filter-868-915-mhz-for-lora-flarm-booster-helium-hotspot-network-miner-hnt-filter-868mhz-sma-7m-4m-saw-cavity-filter.html](https://www.callboost.cn/product-callboost-cavity-filter-868-915-mhz-for-lora-flarm-booster-helium-hotspot-network-miner-hnt-filter-868mhz-sma-7m-4m-saw-cavity-filter.html)

Can often be bought on Aliexpress or they can be contacted via mail for custom qoute.

Price: varies, expect 60-80EUR + shipping

![Callboost 868Mhz filter with 4M bandwidth spectrum analyzer test](callboost-4m-868mhz/callboost-4m-868mhz-tinysa.png)

## Generic AliExpress "Band Pass Filter"

Source: [https://www.aliexpress.com/item/1005007509062592.html](https://www.aliexpress.com/item/1005007509062592.html)

Price: ~17 USD + shipping
![alt text](generic-aliexpress-passband-filter-1/generic-aliexpress-passband-filter-1-testsetup.jpeg)

![Generic 868MHz passband filter spectrum analyzer test](generic-aliexpress-passband-filter-1/generic-aliexpress-passband-filter-1-tinysa.png)